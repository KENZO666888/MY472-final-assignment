...
## 4.Analysis
*The source data table displayed here do not directly enter the analysis, so it is not named and we use head function so it will only display the first 6 rows.I will directly display the imported results without doing any processing on the data source table here to visually display my data source.

UK POLICE SOURCE DATASET:
This is the source table mentioned below. Get uk police official website source data from API,The specific attribute information of each case will be generated here, which will be classified according to the specific attributes for the following articles.
...
```
ONS DATASET:
These 3 tables import UK population distribution from a file of census data obtained from ONS.Three tables will be created here. The first table is the overall population information, which is used to analyze the relative ratio of specific groups to the total population. The second and third tables are gender information separated by men and women, which is used for gender analysis.(Because Because the data table provided by the ONS official website has many introduction lines at the beginning, it is recognized by R as empty lines.We can see that the structure here is very confusing, which will be dealt with later.)
